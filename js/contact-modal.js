  ...
  <div class="modal-content">
    ...
    <div id="bookingSuccess" class="booking-step">
       <div class="success-wrap">
          ...
          <h2 class="modal-title">You're booked!</h2>
          <p class="modal-desc">I've sent a calendar invite to <span id="successEmail" style="font-weight: 600;"></span>. Talk soon!</p>
...
const initModalLogic = () => {
  ...
  // Open triggers
  document.querySelectorAll('.openBookingBtn, .nav-cta').forEach(btn => {
    btn.addEventListener('click', (e) => {
      // If it's the nav-cta, it might be a mailto. We override.
      if (btn.classList.contains('nav-cta') || btn.classList.contains('openBookingBtn')) {
        e.preventDefault();
  ...
  document.querySelectorAll('.modal-back').forEach(btn => {
    btn.onclick = () => {
      ...
      if (current === 'bookingStep2') showStep('bookingStep1');
  ...
  if (form) {
    form.onsubmit = async (e) => {
      ...
      try {
        const res = await fetch('/.netlify/functions/book-call', {
          method: 'POST',
        ...
        alert('Booking failed. Please email me directly!');
      } finally {
        btn.disabled = false;
